...
describe("recordLaunch — input validation", () => {
  ...
  it("rejects empty claimers array", async () => {
    const { recordLaunch } = await import("./launch-integration");
    await expect(recordLaunch(baseInput([]))).rejects.toThrow(
      /claimers.length must be 1..100/,
    );
  });
  ...
  it("rejects more than 100 claimers", async () => {
    ...
    await expect(recordLaunch(baseInput(tooMany))).rejects.toThrow(
      /claimers.length must be 1..100/,
    );
  });
  ...
  it("rejects bps sum != 10000", async () => {
    const { recordLaunch } = await import("./launch-integration");
    await expect(
      recordLaunch(baseInput([claimer(0, 5000), claimer(1, 4000)])),
    ).rejects.toThrow(/sum of initialBps must be 10000/);
  });

  it("rejects bps value out of range", async () => {
    const { recordLaunch } = await import("./launch-integration");
    await expect(recordLaunch(baseInput([claimer(0, 10001)]))).rejects.toThrow(
      /initialBps must be in/,
    );
  });
  ...
  it("rejects duplicate slot indices", async () => {
    const { recordLaunch } = await import("./launch-integration");
    await expect(
      recordLaunch(baseInput([claimer(0, 5000), claimer(0, 5000)])),
  ...
  it("rejects sparse slot indices (must be dense [0..N-1])", async () => {
    const { recordLaunch } = await import("./launch-integration");
    await expect(
      recordLaunch(baseInput([claimer(0, 5000), claimer(2, 5000)])),
  ...
  it("rejects duplicate claimer pubkeys even with distinct slots", async () => {
    ...
    await expect(recordLaunch(baseInput([a, b]))).rejects.toThrow(
      /duplicate claimer pubkey/,
    );
  });
  ...
  it("accepts a valid minimal input (1 claimer at 10000 bps)", async () => {
    ...
    const result = await recordLaunch(baseInput([claimer(0, 10000)]));
